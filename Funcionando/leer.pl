...
%  Metodo de resolucion hecho por mi
...
demostradorBasadoResolucion(Entrada):- leer(Entrada,L),writeln(L), funcion(L,G), write('Lista de literales = '), writeln(G),resolucion(G).
